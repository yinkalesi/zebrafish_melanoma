function [x0,place_dist,place_rad,dist0,prim_list,meta_list,inferred_data,size_projections] = getOriginSizes_noShed_v1_0(key,data,prim_acceptance_thresh)
%% getOriginSizes_noShed_v1_0
%  version 1.0
...
%% Version History
%  1.0: from getOriginSizes_v1_0,removing shedding from size trajectory
%  calculation
...
% create key with zero shedding parameter
changes = struct('SHED_PARAMETER1',0,'SHED_PARAMETER2',0,...
    'CTC_PARAMETER1',0,'SHED_META_RATIO',1e3,'CTC_PARAMETER2',0,...
    'ORIGIN_TIME_CALCULATOR',@getOriginSizes_v1_0);
withShed_key = key;
key = changeKey_v3_1(withShed_key,changes);

x0 = key.TIME_ZERO_SIZES;
place_dist = key.PLACEMENT_DIST;
place_rad = key.PLACEMENT_RADIUS;
dist0 = key.TIME_ZERO_DIST;
prim_list = key.TIME_ZERO_ORIGIN;
meta_list = key.META_ORIGIN;
inferred_data = key.inferred_data;
size_projections = key.SIZE_PROJECTIONS;
...
%                 birth_times(j) = -1;
%             else
%                 [traj,ttraj] = getSizeTrajectory_AlwaysSmaller_v4_0(key,x_ti(j),-dt,ts);
%                 if(traj(end) < x_ti(j))
