...
% ***MODIFIED using CreateNestedCFS_v1_1***
function [tumors,totals] = CreateFishStructNested_Silja_v2_0(start_step,fish_to_skip,show,batch,baseDir,timePoint,fish_specs,...
...
BYTIME = CreateFishStruct_Silja_step1_v2_0(baseDir,timePoint,batch,...
implantSize,implantSizeName,location);
end
...
if(start_step <= step_num)
% ***MODIFIED using CreateNestedCFS_v1_1***
 CreateFishStruct_step7_v2_3(fish_struct_save_name,show);
end
...
if(start_step <= step_num)
% ***MODIFIED using CreateNestedCFS_v1_1***
 CreateFishStruct_identify_byNumber_v1_0(1);
end
...
if(start_step <= step_num)
% ***MODIFIED using CreateNestedCFS_v1_1***
 CreateFishStruct_gender_noData_v1_0();
end
...
% ***INSERTED using CreateNestedCFS_v1_1***
...
% ***INSERTED using CreateNestedCFS_v1_1***
function [ ] = CreateFishStruct_step7_v2_3(name,show)
...
[spine1,spine_func1] = return_spine_outline_mod6(bf1(:,:,1),body1,body1_end,eye1_coor,gfp1,out_name1);
[spine2,spine_func2] = return_spine_outline_mod6(bf2(:,:,1),body2,body2_end,eye2_coor,gfp2,out_name2);
[spine3,cspine_func1] = return_spine_outline_mod6(rgb1(:,:,1),body3,body3_end,eye3_coor,gfp1,out_name3);
[spine4,cspine_func2] = return_spine_outline_mod6(rgb2(:,:,1),body4,body4_end,eye4_coor,gfp2,out_name4);
...
% ***INSERTED using CreateNestedCFS_v1_1***
function [ ] = CreateFishStruct_identify_byNumber_v1_0(show)
%% CreateFishStruct_identify_byNumber_v1_0
%  Version 1.0
...
%  1.0: derived from CreateFishStruct_identify_v2_1
...
% make lookup matrix based on data included in BYTIME
viable_lookup_file = true;
tmax_lkup = length(BYTIME);
nfish_lkup = length(BYTIME(end).dataAt{1});
lookups = zeros(nfish_lkup,tmax_lkup);
% populate lookups
for i = 1:tmax_lkup
for fid = BYTIME(end).dataAt{i}
lookups(fid,i) = fid;
end
end

% still need to create config for backwards compatibility
% config connects the indices of the same fish in dataAt matrices
config = cell(1,length(BYTIME)-1);
if(viable_lookup_file)
for t = 1:tmax_lkup-1
dataAt0 = BYTIME(end).dataAt{t};
dataAt1 = BYTIME(end).dataAt{t+1};
conf = zeros(1,length(dataAt1));
for dai = 1:length(dataAt1)
fid = dataAt1(dai);
% we know fish can be distinguised by fid in this case
conf(dai) = find(dataAt0==fid,1);
end
config{t} = conf;
end
end
...
title(['Fish (t' num2str(t) ', ' num2str(conf_ff) ') GFP1']);
subplot(2,2,2);
...
% ***INSERTED using CreateNestedCFS_v1_1***
function [ ] = CreateFishStruct_gender_noData_v1_0()
%% CreateFishStruct_gender_noData_v1_0
%  Version 1.0
%  Author: Adeyinka Lesi
%  Date: 3/15/18
%  Project: Fish Image Analysis
%% Version History
%  1.0: just assign same gender to everything for compatibility reasons

disp('Step 10.6: Assign gender of fish');
tic;
