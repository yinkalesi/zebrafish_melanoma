function [rate_funcs,string_form] = getRates_Gompertz_v2_0(key)
%% getRates_Gompertz_v2_0
%  Version 2.0
...
%% VERSION HISTORY
%  2.0: updated version of gompertz growth rate

params = key.PARAMETERS;
rate_funcs = struct();
rate_funcs.growth = @(x) params.growth1*x.*log(params.growth2./x);
...
% first derivative
rate_funcs.growth_deriv = @(x) params.growth1.*(log(params.growth2./x)-1);
...
% second derivative
rate_funcs.growth_2deriv = @(x) -params.growth1./x;
...
rate_funcs.size1_ratio = params.growth1*log(params.growth2)/(params.growth1*log(params.growth2)+params.death1);
...
if(key.USING_DEATH_CUTOFF)
    if(isfield(key,'USING_DEATH_STARTER') && key.USING_DEATH_STARTER)
        format = ['k_g=%1.2e\\cdot{}x\\cdot{}ln({%1.2e}/x), ' ...
...
switch parname
    case 'growth1'
        pardevs = x.*log(pars.growth2./x);
        iszero = false;
    case 'growth2'
        pardevs = pars.growth1*x./pars.growth2;
        iszero = false;
    case 'death1'
...
switch parname
    case 'growth1'
        pardevs = pars.death1*log(pars.growth2)/(pars.growth1*log(pars.growth2)+pars.death1)^2;
        iszero = false;
    case 'growth2'
        pardevs = pars.death1*pars.growth1/pars.growth2/(pars.growth1*log(pars.growth2)+pars.death1)^2;
        iszero = false;
    case 'death1'
        pardevs = -pars.growth1*log(pars.growth2)/(pars.growth1*log(pars.growth2)+pars.death1)^2;
        iszero = false;
    case 'death2'
        pardevs = 0;
    case 'shed1'
