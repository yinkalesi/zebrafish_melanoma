...
%  2.0: using later instance of primary to predict size
%  2.1: data extrapolation labeling...
%  3/31/21: made it so place_dist/rad at time 1 is set based on values in
%  key.PLACEMENT_DIST/RADIUS

function [x0,place_dist,place_rad,dist0,prim_list,meta_list,inferred_data,size_projections] = getOriginSizes_v2_1(key,data,prim_acceptance_thresh)
...
% check if data has some extrapolated data points already
if(isfield(data,'extrapol_label'))
    extrapol_label = data.extrapol_label;
else
    extrapol_label = false(size(data.x));
end
...
extp{1} = extrapol_label(data.selector{1});
...
for n = 2:nt
    ...
    extp{n} = extrapol_label(data.selector{n});
...
temp_data = struct('x',cell(1,nt),'dist',cell(1,nt),'ccdf',...
    ...
    'detect_time',cell(1,nt),'origin_time',cell(1,nt),...
    'origin_time_corr',cell(1,nt),'meta_loc',cell(1,nt),...
    'origin_time_loc',cell(1,nt));

x0 = zeros(x0_size,1);
dist0 = zeros(x0_size,1);
prim_list = zeros(x0_size,4);
known_list = zeros(x0_size,6);
meta_list = zeros(x0_size,6);
...
for i = 1:nt
    ...
    if(isempty(i_above))
        ...
        if(i == 1)
            ...
            prim_list(nx0+1:nx0+nai,:) = [ones(nai,1)*i i_above x_ti(i_above) extp{i}(i_above)];
            ...
            prim_list(nx0+1:nx0+nai,:) = [ones(nai,1)*i prim_sel prim_sizes extp{i}(prim_sel)];
            ...
            known_list(nkwn+1:nkwn+nki,:) = [ones(nki,1)*i known_sel known_sizes extp{i}(known_sel) known_dist known_origin_time];
            nkwn = nkwn+nki;
            ...
            meta_list(nmet+1:nmet+nmi,:) = [ones(nmi,1)*i meta_sel meta_sizes extp{i}(meta_sel) meta_dist meta_origin_time];
            nmet = nmet+nmi;
...
% save backtrack_sizes and origin times
size_projections = struct('backtrack_sizes',backtrack_sizes,'origin_times',origin_times);

% keep track of origin_times of nominally 'the same' tumors at each time
fund_inc = min(nonzeros(dist{1}));
norm_ntum = floor(ntum_inc/fund_inc);
orig_t_corr = -1*ones(norm_ntum,nt);
orig_t_loc = zeros(norm_ntum,nt);
...
for i = 1:nt
    ...
    prim_i_lab = prim_list(prim_i,4);
    meta_i_lab = meta_list(meta_i,4);
    known_i_lab = known_list(known_i,4);
    xi_lab = [ones(n_infmeta,1)*2; ones(n_infprim,1)*1; meta_i_lab; prim_i_lab; known_i_lab];
    xi_det_time = zeros(size(xi_lab));
    xi_org_time = zeros(size(xi_lab));
    xi_meta_loc = zeros(size(xi_lab));
    xi = zeros(size(xi_lab));
    disti = zeros(size(xi_lab));
    nxi = 0;
    % look at tumor detected at time i
    xi(end-nxi-n_known+1:end-nxi) = backtrack_sizes{i}(known_list(known_i,2),i); % add already id'ed tumors
    disti(end-nxi-n_known+1:end-nxi) = dist{i}(known_list(known_i,2),1);
    xi_org_time(end-nxi-n_known+1:end-nxi) = origin_times{i}(known_list(known_i,2),1);
    nxi = nxi+n_known;
    xi(end-nxi-n_newprim+1:end-nxi) = backtrack_sizes{i}(prim_list(prim_i,2),i); % add newly id'ed primaries (not inferred)
    disti(end-nxi-n_newprim+1:end-nxi) = dist{i}(prim_list(prim_i,2),1);
    xi_org_time(end-nxi-n_newprim+1:end-nxi) = origin_times{i}(prim_list(prim_i,2),1);
    nxi = nxi+n_newprim;
    xi(end-nxi-n_newmeta+1:end-nxi) = backtrack_sizes{i}(meta_list(meta_i,2),i); % add newly id'ed metastases
    disti(end-nxi-n_newmeta+1:end-nxi) = dist{i}(meta_list(meta_i,2),1);
    xi_org_time(end-nxi-n_newmeta+1:end-nxi) = origin_times{i}(meta_list(meta_i,2),1);
    xi_meta_loc(end-nxi-n_newmeta+1:end-nxi) = find(meta_i);
    nxi = nxi+n_newmeta;
    xi_det_time(end-nxi+1:end) = ones(nxi,1)*i;
    
    infp_inds = find(infp_i); % add inferred primaries 
    for j = 1:n_infprim
        ...
        xi_det_time(end-nxi) = prim_list(infp_inds(j),1);
        xi_org_time(end-nxi) = origin_times{prim_list(infp_inds(j),1)}(prim_list(infp_inds(j),2),1);
        nxi = nxi+1;
    end
    infm_inds = find(infm_i); % add inferred metastases 
    for j = 1:n_infmeta
        ...
        xi_det_time(end-nxi) = meta_list(infm_inds(j),1);
        xi_org_time(end-nxi) = origin_times{meta_list(infm_inds(j),1)}(meta_list(infm_inds(j),2),1);
        xi_meta_loc(end-nxi) = infm_inds(j);
        nxi = nxi+1;
    end
    ...
    disti = disti(ord);
    xi_lab = xi_lab(ord);
    xi_det_time = xi_det_time(ord);
    xi_org_time = xi_org_time(ord);
    xi_meta_loc = xi_meta_loc(ord);
    ...
    % store correlated origin times
    curi = 0;
    flip_ccdfi = flipud(ccdfi);
    for nn = 1:norm_ntum
        sni = find(flip_ccdfi((curi+1):end)>=nn*fund_inc,1,'first')-1; % find next tumor position
        if(~isempty(sni))
            orig_t_corr(nn,i) = xi_org_time(end-sni-curi+1);
            orig_t_loc(nn,i) =  xi_meta_loc(end-sni-curi+1);
            curi = sni+curi;
        else
            break;
        end
    ...
    temp_data(i).detect_time = xi_det_time;
    temp_data(i).origin_time = xi_org_time;
    temp_data(i).origin_time_corr = orig_t_corr;
    temp_data(i).meta_loc = xi_meta_loc;
    temp_data(i).origin_time_loc = orig_t_loc;
