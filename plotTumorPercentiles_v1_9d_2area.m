...
%  1.8_2area: comparison with mm with shifted immunity time
%  1.9d: change of size limit, day 9 immunity
...
% not used yet
savedir = 'C:\Users\Yinka\Documents\CCNY\Rumschitzki Group\Continuous Tumor Growth Model\paper1_plots_052322\';
...
modres_hilo2 =load('ct10_lt015_ht04_021720_final_hilo_mm_day9Immunity_011822_mod_runModelHelper_mm_sensitivity_v1_2b.mat');
...
plotWithTrajectoriesAreaError4(gdata,2,'',[2:19]-1,hilo_m_nr,hilo_m_nr2,[large_tumor_thresh inf],0)
print(sprintf('%sv%i_above_2e5_zf19_m_areaerror_mm2area',savedir,version_num),'-djpeg');
plotWithTrajectoriesAreaError4(gdata,2,'',[2:19]-1,hilo_f_nr,hilo_f_nr2,[large_tumor_thresh inf],1)
print(sprintf('%sv%i_above_2e5_zf19_f_areaerror_mm2area',savedir,version_num),'-djpeg');

% calculate crossover size range
crossover_list = zeros(length(hilo_f_nr2),2);
for ic = 1:size(crossover_list,1)
    crossover_list(ic,1) = calculateCrossoverSizes_v2_0(hilo_m_nr2{ic}.key);
    crossover_list(ic,2) = calculateCrossoverSizes_v2_0(hilo_f_nr2{ic}.key);
end
dlmwrite(sprintf('%sv%i_mf_crossoverSizes',savedir,version_num),crossover_list);

plotWithTrajectoriesSensitivityRange(gdata,2,'',[2:15]-1,hilo_m_nr2,hilo_f_nr2,[large_tumor_thresh inf],crossover_list);
print(sprintf('%sv%i_zf19_sensitivityRange',savedir,version_num),'-djpeg');
...
if group_sel == 1
    axlims = [0 14 max_val-6.0 max_val];
elseif group_sel == 0
    axlims = [0 14 max_val-6.0 max_val];
else
    axlims = [0 21 max_val-6.0 max_val];
end
axis(axlims);
...
if(gender==0)
    ...
    leg_text{2} = sprintf('Prediction, Day 9 immunity, x0 = %2.1e',x1);
    first_young = find(ismember(in_range1,find(age_crit1==1)),1,'first');
    first_old = find(ismember(in_range1,find(age_crit1==2)),1,'first');
    for ii = 3:length(leg_text)
        if(ii==first_young+2)
...
function [] = plotWithTrajectoriesAreaError4(gdata,group_sel,title_text,tsel,hilo1,hilo2,size_range,gender)
% plotting theory curve along side individual trajectories
% sort tumor curves by ages and color differently
% 1/7/22: uing multiple starting sizes for theory shaded region

% plot male and female together
% rnr = 0 for rad and 1 for norad 
%figure('position',[769.8000 41.8000 766.4000 740.8000]);
figure('units','normalized','outerposition',[.05 .05 .50 .60]);
fontsizes = [9 12 12 10]*2; 
if(group_sel == 0)
    st = gdata(1).trad(1,1);
    tout1 = gdata(1).tout_r(tsel);
...
if(~isempty(in_range1))
    ...
    med1 = median(log_rad1(in_range1,:)/log(10),1);
    avg1 = mean(log_rad1(in_range1,:)/log(10),1);
    max1 = max(log_rad1(in_range1,:)/log(10));
    stdfac1 = 2;%10.^std(log_rad1(in_range1,1)/log(10));
else
    t1 = [1];
    med1 = log10(2e5);
    avg1 = log10(2e5);
...
if(~isempty(in_range2))
    ...
    med2 = median(log_rad2(in_range2,:)/log(10),1);
    avg2 = mean(log_rad2(in_range2,:)/log(10),1);
    max2 = max(log_rad2(in_range2,:)/log(10));
    stdfac2 = 2;%10.^std(log_rad2(in_range2,1)/log(10));
else
    t2 = [1];
    med2 = log10(2e5);
    avg2 = log10(2e5);
...
if group_sel == 0
    ...
    %     x1 = 10^(0.5*(med1(1)+med2(1)));
    %     x2 = 3e6;
    xmax = 10.^max(max1(1:6)); %     xmax = 10.^(max(max1(1),max2(1)));
    x1 = 5.66e6;
    x2 = 17067200;
    t_imm1 = 4.2540;
    t_imm2 = 9.254;
...
hilo1 = {hilo1{1} hilo1{1}};
hilo2 ={hilo2{1} hilo2{1}};

% get proper starting sizes to match Day 1 size
mid_key = hilo1{1}.key;
x0 = [getDayTSize(hilo2{1}.key,x2,t_imm1) getDayTSize(hilo1{1}.key,x1,t_imm1)];
x0mid = 10.^mean(log10(x0));

[mid_traj1,mid_t1] = getSizeTrajectory_v3_3(mid_key,x0mid,dt,tf);
lmid_traj1 = log10(mid_traj1);

hilo_trajs = zeros((length(hilo1)-1)*length(x0),length(mid_traj1));

for ii = 1:length(x0)
    for jj = 1:(length(hilo1)-1)
        hh = (ii-1)*(length(hilo1)-1)+jj;
        [hilo_trajs(hh,:),~] = getSizeTrajectory_v3_3(hilo1{jj+1}.key,x0(ii),dt,tf);
    end
end
...
% second time with second set of parameters
mid_key = hilo2{1}.key;
x0 = [getDayTSize(hilo2{1}.key,x2,t_imm2) getDayTSize(hilo1{1}.key,x1,t_imm2)];
x0mid = 10.^mean(log10(x0));

[mid_traj2,mid_t2] = getSizeTrajectory_v3_3(mid_key,x0mid,dt,tf);
lmid_traj2 = log10(mid_traj2);

hilo_trajs = zeros((length(hilo2)-1)*length(x0),length(mid_traj1));

for ii = 1:length(x0)
    for jj = 1:(length(hilo2)-1)
        hh = (ii-1)*(length(hilo2)-1)+jj;
        [hilo_trajs(hh,:),~] = getSizeTrajectory_v3_3(hilo2{jj+1}.key,x0(ii),dt,tf);
    end
end
...
% different male and female plot
if(gender==0)
    h = plot(0,0,'g',0,0,'b');%plot(mid_t1,lmid_traj1,'g--',mid_t2,lmid_traj2,'b--','LineWidth',1.5);
    hold on;
    ...
    leg_text = cell(1,2+length(in_range1));
    leg_text{1} = sprintf('Prediction, Day 4 immunity');
    leg_text{2} = sprintf('Prediction, Day 9 immunity');
    first_young = find(ismember(in_range1,find(age_crit1==1)),1,'first');
    first_old = find(ismember(in_range1,find(age_crit1==2)),1,'first');
    for ii = 3:length(leg_text)
        if(ii==first_young+2)
...
elseif group_sel == 0
    ...
    axlims = [0 tf max_val-6.0 max_val];
end
axis(axlims);
...
function [] = plotWithTrajectoriesSensitivityRange(gdata,group_sel,title_text,tsel,hilo_m,hilo_f,size_range,crossover_list)
% plotting theory curve along side individual trajectories
% sort tumor curves by ages and color differently

% plot male and female together
% rnr = 0 for rad and 1 for norad 
fig = figure('position',[769.8000 41.8000 766.4000 740.8000]);
fontsizes = [9 12 12 10]*2; 
if(group_sel == 0)
    st = gdata(1).trad(1,1);
    tout1 = gdata(1).tout_r(tsel);
...
% get proper starting sizes to match Day 1 size
mid_key = hilo_m{1}.key;
x0 = getDayTSize(mid_key,x1,t_imm);

[mid_traj1,mid_t1] = getSizeTrajectory_v3_3(mid_key,x0,dt,tf);
lmid_traj1 = log10(mid_traj1);

hilo_trajs = zeros(length(hilo_m)-1,length(mid_traj1));

for hh = 1:size(hilo_trajs,1)
    [hilo_trajs(hh,:),~] = getSizeTrajectory_v3_3(hilo_m{hh+1}.key,x0,dt,tf);
end
...
% second time with second set of parameters
mid_key = hilo_f{1}.key;
% same x0 as male; x0 = getDayTSize(mid_key,x1,t_imm);

[mid_traj2,mid_t2] = getSizeTrajectory_v3_3(mid_key,x0,dt,tf);
lmid_traj2 = log10(mid_traj2);

hilo_trajs = zeros(length(hilo_f)-1,length(mid_traj1));

for hh = 1:size(hilo_trajs,1)
    [hilo_trajs(hh,:),~] = getSizeTrajectory_v3_3(hilo_f{hh+1}.key,x0,dt,tf);
end
...
% male and female prediction curves on same plot

h = plot(mid_t1,lhi_traj1,'g-',mid_t1,lmid_traj1,'g--',mid_t1,llo_traj1,'g-',...
    mid_t2,lhi_traj2,'b-',mid_t2,lmid_traj2,'b--',mid_t2,llo_traj2,'b-',...
    [0 mid_t1(end)],[1 1]*log10(crossover_list(1,1)),'g:',[0 mid_t1(end)],[1 1]*log10(crossover_list(1,2)),'b:','LineWidth',1.5);
arr1 = annotation('doublearrow');
arr2 = annotation('doublearrow');
...
leg_text = cell(1,8);
leg_text{1} = sprintf('Male Prediction +5%%');
leg_text{2} = sprintf('Male Prediction');
leg_text{3} = sprintf('Male Prediction -5%%');
leg_text{4} = sprintf('Female Prediction +5%%');
leg_text{5} = sprintf('Female Prediction');
leg_text{6} = sprintf('Female Prediction -5%%');
leg_text{7} = 'Male Crossover Size';
leg_text{8} = 'Female Crossover Size';
...
arr1.Parent = fig.CurrentAxes;
arr1.X = [1 1];
arr1.Y = log10([min(crossover_list(:,1)) max(crossover_list(:,1))]);
arr1.Color = [0 1 0];

arr2.Parent = fig.CurrentAxes;
arr2.X = [1 1];
arr2.Y = log10([min(crossover_list(:,2)) max(crossover_list(:,2))]);
arr2.Color = [0 0 1];
...
function [x0] = getDayTSize(key,x1,t)
    a = key.GROWTH_PARAMETER1;
    b = key.GROWTH_PARAMETER2;
    x0 = (x1.^(1-b)-a*(1-b)*t).^(1/(1-b));
end